...
interface Conversation {
  ...
  demandeur_id: string;
  statut: string;
  demande?: { titre: string; auteur: string };
}
...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchConv = async () => {
      const { data } = await supabase
        .from("conversations")
        .select("*, demande:demande_id(titre, auteur)")
        .eq("id", id)
        .single();
  ...
  // Après paiement, proposer l'adresse automatiquement
  useEffect(() => {
    if (conversation?.statut === "payé" && userAddress && user?.id === conversation.demandeur_id) {
      setShowAddressPrompt(true);
    }
  }, [conversation, userAddress, user]);
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-foreground">{conversation?.demande?.titre || "Conversation"}</p>
            <p className="text-xs text-muted-foreground">
              {conversation?.statut === "payé" ? "✅ Payé" : "💬 En cours"}
            </p>
          </div>
        </div>
      </header>
        ...
        {messages.map((msg, i) => (
          ...
          >
            <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm ${
              msg.is_auto
                ? "bg-primary/10 text-primary border border-primary/20 rounded-2xl"
                : isMe(msg.sender_id)
      ...
      <AnimatePresence>
        {showAddressPrompt && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
            className="fixed bottom-20 left-4 right-4 bg-card border border-primary/30 rounded-2xl p-4 shadow-lg z-10"
          >
