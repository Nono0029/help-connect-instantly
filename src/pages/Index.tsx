        ...
        <div className="flex gap-2 overflow-x-auto px-4 pb-3 scrollbar-hide">
          {categories.map(cat => (
            <button key={cat} onClick={() => setSelectedCat(cat)}
              className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${selectedCat === cat ? "bg-primary text-primary-foreground shadow-sm" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 px-4 pt-4 pb-24 space-y-3">
        {filtered.length === 0 && (
          <div className="text-center text-muted-foreground py-20">
            <Illu name="empty" className="w-48 mx-auto mb-4 opacity-60" />
            ...
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <button onClick={(e) => toggleLike(d.id, e)} className="p-1">
                  <Heart className={`w-5 h-5 transition-colors ${likedIds.includes(d.id) ? "text-red-500 fill-red-500" : "text-muted-foreground"}`} />
                </button>
              </div>
