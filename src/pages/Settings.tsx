import { ArrowLeft, User, Moon, Sun, ChevronRight, Shield, Bell, Heart, ShoppingBag, HelpCircle, LogOut, Star, Briefcase, FileText, Scale } from "lucide-react";
...
const Settings = () => {
  ...
  const menuSections = [
    ...
    },
    {
      title: "Prestataire",
      items: [
        { icon: Briefcase, label: "Devenir prestataire", desc: "Propose tes services", action: () => navigate("/become-pro") },
      ],
    },
    {
      title: "Aide",
      items: [
        { icon: HelpCircle, label: "Centre d'aide", desc: "FAQ et support", action: () => navigate("/aide") },
        { icon: FileText, label: "Confidentialité & légal", desc: "RGPD, CGU, mentions", action: () => navigate("/privacy") },
            ...
            <div className="bg-card rounded-2xl border border-border overflow-hidden divide-y divide-border">
              {section.items.map(item => (
                <button key={item.label} onClick={item.action} className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-secondary/50 transition-colors">
                  ...
                  {(item as any).toggle ? (
                    <div className={`w-11 h-6 rounded-full transition-all flex items-center px-0.5 ${theme === "dark" ? "bg-primary" : "bg-secondary"}`}>
